-- server/main.lua
-- Antique: the target grants a raw, unidentified antique item
...
-- swept this player's held-but-unregistered antiques since the last
...
local function loadPlayer(src, citizenid)
	local row = MySQL.single.await('SELECT owned, claimed FROM vay_zukan_antique WHERE citizenid = ?', { citizenid })
	PlayerCache[src] = {
		citizenid = citizenid,
...
local function savePlayer(src)
	...
	MySQL.update(
		[[INSERT INTO vay_zukan_antique (citizenid, owned, claimed) VALUES (?, ?, ?)
...
-- Wipes any held-but-unregistered antiques the first time this citizenid is
...
lib.callback.register('vay_zukan_antique:getSync', function(src)
	local cache = PlayerCache[src]
	if not cache then
...
--------------------------------------------------------------------------
-- pickup (grants the raw antique item, does NOT register by itself)
--------------------------------------------------------------------------

RegisterNetEvent('vay_zukan_antique:pickup', function(itemId)
	local src = source
	local cache = PlayerCache[src]
	...
	if added then
		TriggerClientEvent('vay_zukan_antique:itemPickedUp', src, { ItemId = item.Id, Name = item.Name })
	end
end)

--------------------------------------------------------------------------
-- NPC: registers every held antique, then grants any newly-completed
-- sheet's reward, in the same interaction.
--------------------------------------------------------------------------

RegisterNetEvent('vay_zukan_antique:talk', function()
	local src = source
	local cache = PlayerCache[src]
	if not cache then
		return
	...
	for sheetIndex, sheet in ipairs(Zukan.Sheets) do
		if not cache.Claimed[sheetIndex] and isSheetComplete(cache.Owned, sheetIndex) then
			...
			if reward.Kind == 'Money' then
				Player.Functions.AddMoney(reward.Account or 'bank', reward.Amount, 'vay-zukan-antique-reward')
			end
	...
	TriggerClientEvent('vay_zukan_antique:turnedIn', src, { Registered = registered, Granted = granted })
end)
