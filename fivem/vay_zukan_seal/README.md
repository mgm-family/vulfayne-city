...
シール集め専用の独立リソースです（10シート × 1シート9個 = 90種）。オーパーツ・骨董品はそれぞれ `vay_zukan_oopart` / `vay_zukan_antique` という別リソースです。
...
- 1シート（9個）揃うと、シールコレクターNPCに話しかけて景品を受け取れます（デフォルトはQBCoreの銀行口座への入金）
