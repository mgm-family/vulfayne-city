-- client/main.lua
-- Registers an ox_target pickup zone for every oopart the player neither
-- owns (registered) nor already carries as a raw relic item, draws a
-- glow marker nearby, spawns the collector NPC (relic hand-in + reward),
-- and drives the NUI album (opened by using the vay_zukan_oopart item).

local QBCore = exports['qb-core']:GetCoreObject()

local MARKER_DRAW_DISTANCE = 25.0
local MARKER_COLOR = { 173, 58, 43 } -- Zukan.ThemeColor (#ad3a2b) as RGB

local State = { Owned = {}, Claimed = {} }
local HeldRaw = {} -- [itemId] = true, currently carrying the unregistered relic
...
local function registerItemZone(item)
	...
	local zoneName = 'vay_zukan_oopart_' .. item.Id
		...
		options = {
			{
				...
				label = ('オーパーツ：%s を拾う'):format(item.Name),
				onSelect = function()
					TriggerServerEvent('vay_zukan_oopart:pickup', item.Id)
				end,
			},
		},
	})
...
-- Glow marker for nearby collectible opearts.
...
--------------------------------------------------------------------------
-- NPC (relic hand-in + reward, same interaction)
--------------------------------------------------------------------------
	...
	exports.ox_target:addLocalEntity(NpcPed, {
		{
			...
			label = 'オーパーツを納品する',
			onSelect = function()
				TriggerServerEvent('vay_zukan_oopart:talk')
			end,
		},
	})
end
...
-- Called by ox_inventory when the vay_zukan_oopart item is used.
...
RegisterCommand('vay_zukan_oopart', function()
	if not initialized then
		return
	end
	if not hasZukanItem() then
		lib.notify({ description = '「オーパーツ図鑑」を持っていないと開けません。', type = 'error' })
		return
	end
	toggleUI()
end, false)
RegisterKeyMapping('vay_zukan_oopart', 'オーパーツ図鑑を開閉', 'keyboard', 'F7')
...
RegisterNetEvent('vay_zukan_oopart:itemPickedUp', function(payload)
...
RegisterNetEvent('vay_zukan_oopart:turnedIn', function(payload)
	...
	if #payload.Registered > 0 then
		lib.notify({ description = ('%d件のオーパーツを図鑑に登録した！'):format(#payload.Registered), type = 'success' })
	else
		lib.notify({ description = '納品できるオーパーツを持っていません。', type = 'error' })
	end
...
local function init()
	...
	local ok, snapshot = pcall(function()
		return lib.callback.await('vay_zukan_oopart:getSync', false)
	end)
...
RegisterCommand('vay_zukan_oopart_getcoord', function()
	local coords = GetEntityCoords(PlayerPedId())
	...
	print(('[vay_zukan_oopart] item Coords: %s | NPC Coords: %s'):format(vec3Text, vec4Text))
	TriggerEvent('chat:addMessage', {
		args = { '^3[vay_zukan_oopart]', ('item: %s\nNPC: %s'):format(vec3Text, vec4Text) },
	})
end, false)
